...
impl StorageProof {
    ...
    pub fn new(proof: Vec<(&str, Option<&str>)>) -> anyhow::Result<Self> {
        ...
        for (proof_node, hash) in proof.into_iter() {
            // Decode hex data.
            let mut proof_node_bytes = hex::decode(proof_node)?;
            let hash = hash.map(|h| hex::decode(h).unwrap());
            ...
            hashes.push(hash.map(|h| slice_to_field_elements(&h)));
        }
