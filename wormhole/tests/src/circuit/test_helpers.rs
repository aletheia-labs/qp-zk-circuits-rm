...
impl TestInputs for CircuitInputs {
    fn test_inputs() -> Self {
        ...
        let root_hash: [u8; 32] = hex::decode(storage_proof::DEFAULT_ROOT_HASH.trim())
            .unwrap()
            .try_into()
            .unwrap();

        let funding_account =
            SubstrateAccount::new(hex::decode(DEFAULT_FUNDING_ACCOUNT).unwrap().as_slice())
                .unwrap();
        let nullifier = Nullifier::new(
            &secret,
            DEFAULT_FUNDING_NONCE,
            &hex::decode(DEFAULT_FUNDING_ACCOUNT).unwrap(),
        );
        ...
        Self {
            ...
            private: PrivateCircuitInputs {
                secret,
                storage_proof: storage_proof::default_storage_proof(),
                funding_nonce: 0,
                funding_account,
                unspendable_account,
            },
